...
public class ShopCartCalculatorSG implements ShopCartCalculator {
	...
	public BigDecimal getSubTotal(MyCart myCart) {
		...
		for( WebShopItem item : shoppingList ) {
			tmp = item.getItemPrice();
			total = total.add( tmp.multiply(new BigDecimal(item.getQuantity())) );
		}
	...
	public BigDecimal getGroundTotal(MyCart myCart, String voucherKey) {
		...
		if ( getVoucherIncludeShippingFeeFlag() ) total = total.add(getShippingFee()).subtract(totalDeductionByBundleDiscount).subtract(totalDeductionByVoucher);
		else total = total.subtract(totalDeductionByBundleDiscount).subtract(totalDeductionByVoucher).add(getShippingFee());
